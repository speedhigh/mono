<template>
  <div class="space-y-[4.38rem] pb-20">
    <!-- banner -->
    <div class="w-full h-[18.75rem] relative flex items-center">
      ...
      <div class="w-[1200px] mx-auto relative">
        <h1 class="text-4xl text-white">{{ t('message.about') }}</h1>
      </div>
    </div>
    <!-- profile -->
    <div class="w-[1200px] mx-auto text-lg leading-8">
      日本HSD国際株式会社は2019年4月に設立されました。本社は福岡にあります。主に医薬品、健康食品、医療機器を取り扱う会社です。安全で信頼できる日本の医薬品や医療機器をパートナーに提供するために、いくつかの外国企業と戦略的パートナーシップを結び、エンドユーザーはアジアのいくつかの国にいます。
    </div>
    <!-- menu -->
    <div class="w-[1200px] mx-auto grid grid-cols-3 gap-x-[2.63rem] gap-y-[2.13rem]">
      <!-- 公司简介 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative" @click="$router.push('/about/company')">
      ...
      <!-- 经营理念 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative border" @click="$router.push('/about/business')">
      ...
      <!-- 药剂师简介 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative" @click="$router.push('/about/pharmacist')">
      ...
      <!-- 沿革历史 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative" @click="$router.push('/about/history')">
        <img :src="aboutD" alt="沿革历史" width="372" height="220" class="w-full h-full rounded-lg">
        <div class="absolute bottom-0 inset-x-0 w-full h-[50px] rounded-b-lg bg-[rgba(0,0,0,0.5)]">
          <p class="text-lg text-white text-center leading-[50px]">{{ t('message.aboutHistory') }}</p>
        </div>
      </div>
      <!-- 公司资质 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative" @click="$router.push('/about/aptitude')">
        <img :src="aboutE" alt="公司资质" width="372" height="220" class="w-full h-full rounded-lg object-cover">
        <div class="absolute bottom-0 inset-x-0 w-full h-[50px] rounded-b-lg bg-[rgba(0,0,0,0.5)]">
          <p class="text-lg text-white text-center leading-[50px]">{{ t('message.aboutusB') }}</p>
        </div>
      </div>
      <!-- 招聘信息 -->
      <div class="w-full h-[13.75rem] rounded-lg cursor-pointer hover:opacity-80 relative" @click="$router.push('/about/recruit')">
        <img :src="aboutF" alt="招聘信息" width="372" height="220" class="w-full h-full rounded-lg object-cover">
        <div class="absolute bottom-0 inset-x-0 w-full h-[50px] rounded-b-lg bg-[rgba(0,0,0,0.5)]">
